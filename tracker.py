...
def run_tracker(project_name, config_filename, headless=False):
    ...
    # 🧩 【重要】パーツJSONを読み込んで、1つの巨大なステップに結合する処理
    combined_steps = []
    ...
    if "include_parts" in master_config:
        ...
        for part_rel_path in master_config["include_parts"]:
            # 各パーツファイルの絶対パスを構築 (例: project/sonysonpo/parts/login.json)
            part_path = os.path.join(project_dir, "parts", part_rel_path)
            
            if os.path.exists(part_path):
                with open(part_path, "r", encoding="utf-8") as f:
                    part_data = json.load(f)
                    # パーツ内の steps 配列を取り出して結合
    ...
    for attempt in range(max_retries):
        ...
        current_time = datetime.now().strftime("%Y%m%d_%H%M") # 例: "20260617_1745"
        
        # JSONからベースのファイル名を取得（なければ設定ファイル名をベースにする）
        base_filename = master_config.get("output_file", config_filename.replace(".json", "_result.json"))
        
        # 頭に日時をドッキングさせ、案件フォルダの outputs 内へ保存
        output_filename = f"{current_time}_{base_filename}"
        output_path = os.path.join(output_dir, output_filename)

        step_error_occurred = False # ステップ実行中にエラーが出たかを管理するフラグ

        with sync_playwright() as p:
            # ⭕ 【大幅強化】Akamai(WAF)の強力なBot検知を騙すための起動引数を追加
            chrome_args = [
                "--disable-blink-features=AutomationControlled", 
                "--use-fake-ui-for-media-stream",
                "--disable-infobars",
                "--window-size=1280,800",
                "--blink-settings=imagesEnabled=true" # ヘッドレス時の「画像読み込みOFF」によるBot判定を回避
            ]
            ...
            # 🌐 context（ブラウザ全体）にリスナーを設定して別ウインドウの通信も追う
            ...
            try:
                ...
                # ページ内のスクリプトやネットワークが落ち着くまで少し手厚く待つ
                page.wait_for_load_state("networkidle", timeout=10000)
            except Exception as e:
                print(f"   ⚠️ 初期ページの読み込み待ちでタイムアウト（続行します）: {e}")
            ...
            for i, step in enumerate(combined_steps):
                ...
                try:
                    # 🔄 ウィンドウの切り替え指示（switch_window）の場合の処理
                    if action == "switch_window":
                        ...
                        if target_name in windows and windows[target_name] is not None:
                            ...
                            page = windows[target_name]
                            page.bring_to_front()
                            latest_opened_window = None # 保管庫を空にする
                        ...
                        continue # 次のステップ（Step）へ進む

                    # 💡 【完全自動レコーダー対応 ＆ フォールバック機能】
                    target_loc = None
                    if selector:
                        target_loc = page.locator(selector).first
                        
                        try:
                            # 🚀 表示（visible）されるまで待つ。見つかれば即座に次へ進む
                            target_loc.wait_for(state="visible", timeout=10000)
                        except Exception as e_timeout:
                            # ⚠️ 10秒待っても見つからない場合、自己修復（曖昧検索）を発動
                            match = re.search(r'text="(.*?)"|:has-text\("(.*?)"\)', selector)
                            if match:
                                fallback_text = match.group(1) or match.group(2)
                                print(f"      ⚠️ セレクタが見つかりません。テキスト『{fallback_text}』で曖昧検索に切り替えます...")
                                target_loc = page.locator(f'text="{fallback_text}"').first
                                target_loc.wait_for(state="visible", timeout=5000)
                            else:
                                raise e_timeout
                    ...
                    if action == "click":
                        try:
                            with context.expect_page(timeout=1500) as new_page_info:
                                target_loc.click(timeout=10000)
                        ...
                        if element_tag == "select":
                            target_loc.select_option(value=value, timeout=10000)
                            print(f"      選択肢 [{value}] をセレクトボックスから選択しました。")
                        else:
                            target_loc.fill(value, timeout=10000)
                    ...
                    try:
                        screenshot_path = os.path.join(output_dir, f"error_step_{i+1}.png")
                        page.screenshot(path=screenshot_path, full_page=True)
                        print(f"      📸 [Debug] エラー時点のスクリーンショットを保存しました: {screenshot_path}")
                    except Exception as screenshot_err:
                        print(f"      ⚠️ スクリーンショット撮影に失敗しました: {screenshot_err}")
